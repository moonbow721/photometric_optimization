...
class Renderer(nn.Module):
    ...
    def render_shape(self, vertices, transformed_vertices, images=None, lights=None, h=None, w=None):
        batch_size = vertices.shape[0]
        if lights is None:
        ...
        if images is None:
            shape_images = shading_images*alpha_images + torch.zeros_like(shading_images).to(vertices.device)*(1-alpha_images)
        else:
            shape_images = shading_images*alpha_images + images*(1-alpha_images)
    ...
    def save_static_obj(self, filename, vertices):
        '''
        vertices: [nv, 3], tensor
        '''
        face_vertices = util.face_vertices(vertices, self.faces.expand(vertices.shape[0], -1, -1)).detach()
        util.save_obj(filename, face_vertices, self.faces[0], textures=None, uvcoords=None, uvfaces=None)
