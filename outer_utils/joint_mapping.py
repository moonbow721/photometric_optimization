...
if __name__ == '__main__':
    print(len(smplx_joints_name))
    for key, value in smplx_joint_part.items():
        print(key, len(value))

    print(len(smpl_to_openpose(use_face_contour=True)))

    print(len(joint_set_openpose['joints_name']))

    # Print the mapping to verify
    print(coco_to_openpose()[:25])
    print()
    print(openpose_to_coco()[:25])